...
export class AuthService {
  ...
  login(credenciales: LoginRequest): Observable<LoginResponse> {
    return this.http
      ...
      .pipe(
        tap((res) => {
          if (!res.error && res.simulatedSession) {
  ...
  /**
   * Registro Real conectado a /api/usuarios
   * El payload debe contener: DNI, Ubigeo, Zona, email, password, username, etc.
   */
  registrar(datos: any): Observable<RegistroUsuarioResponse> {
  ...
  changePassword(datos: ChangePasswordRequest): Observable<ChangePasswordResponse> {
    return this.http.post<ChangePasswordResponse>(`${this.baseUrl}/auth/change-password`, datos);
  }
